# Shipping Sunshine
## Battery Ships, Floating Storage, and Indonesia's Renewable Energy Future
...
Battery ships offer an unconventional answer to this conventional problem.
...
The realistic answer is a portfolio. Cables for trunk routes. Local PV where it fits. Hydrogen where chemistry and scale justify the conversion losses. Battery ships for the last mile -- the difficult, expensive, scattered archipelagic fringe where nothing else reaches.

**Major cities were never the right application for battery ships.**
...
## The Island Battery Ship: A Different Vessel
...
Here is the critical insight: battery ships need not collect their charge from Australia. They could collect it from **cable hubs** -- larger islands that receive bulk power via submarine cable, large-scale PV farms, or geothermal plants, and redistribute it to surrounding smaller islands by sea.
...
2. **Hub charging stations** on these islands charge battery ships, drawing from the cable or from local renewables
...
The economics that condemn battery ships when competing with diesel at mainland prices become highly favourable when the comparison is with diesel on a remote island. This is the pivot point of the entire argument.
...
| | Remote Island Diesel | Battery Ship (hub model) |
|---|:---:|:---:|
...
So far, this paper has treated battery ships as transport vessels -- moving energy from Point A to Point B. But a battery ship docked at a port is, functionally, a grid-scale battery installation. This second role may prove as important as the first.
...
Battery ships are not merely a technology to be imported. They are an opportunity to build a new industrial ecosystem on Indonesian soil -- one that generates manufacturing jobs, develops technical capability, and gives the new capital an economic reason to exist.
...
| Battery ship construction | Balikpapan/Samarinda yards | Shipyard trades | Steel, electrical systems |
| Charging hub operation | Balikpapan port | Port operations, electrical | PV-generated electricity |
| Fleet operations | Makassar Strait and eastern routes | Maritime crew, port workers | Charged battery ships |
...
For battery ships -- and the broader renewable transformation they serve -- to move from concept to reality, six things need to happen. Some are technological. Most are political.
